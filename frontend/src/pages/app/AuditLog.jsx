      ...
      <div style={{ display: 'flex', gap: 4, alignItems: 'flex-end', height: 60 }}>
        {data.map((d, i) => (
          <div key={i} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 3 }}>
            <div style={{
              width: '100%', background: `rgba(192,57,43,${0.3 + (d.count / max) * 0.7})`,
...
function LogRow({ log, isNew }) {
  ...
  return (
    <div style={{
      ...
      padding: '9px 14px',
      background: isNew ? 'rgba(192,57,43,0.06)' : '#0F1018',
      borderRadius: 6,
  ...
  return (
    <div style={{ padding: '24px 28px' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 20, flexWrap: 'wrap', gap: 10 }}>
        <div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 4 }}>
            ...
            {liveCount > 0 && (
              <span style={{ fontSize: '0.65rem', color: '#C0392B', fontFamily: 'JetBrains Mono', background: 'rgba(192,57,43,0.1)', padding: '2px 8px', borderRadius: 10 }}>
                +{liveCount} new
              </span>
            )}
          </div>
          ...
          {['', 'auth', 'case', 'ai', 'intel', 'evidence', 'import', 'admin'].map(cat => (
            <button key={cat} onClick={() => setCategory(cat)}
              style={{
                ...
                background: category === cat ? `${CAT_COLOR[cat] || 'rgba(192,57,43,1)'}18` : 'transparent',
                borderColor: category === cat ? `${CAT_COLOR[cat] || '#C0392B'}50` : 'rgba(255,255,255,0.08)',
                color: category === cat ? (CAT_COLOR[cat] || '#C0392B') : '#71717A',
              }}>
      ...
      <div style={{ fontSize: '0.72rem', color: '#71717A', marginBottom: 10, fontFamily: 'JetBrains Mono' }}>
        ...
        {!paused && <span style={{ color: '#C0392B' }}> · auto-refreshing</span>}
      </div>
