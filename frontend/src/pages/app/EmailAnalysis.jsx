          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            {!result ? (
              ...
              <>
                {/* AI Verdict */}
                {aiData.verdict && (
                  <div className="at-card" style={{ padding: 14, borderLeft: `2px solid ${aiData.verdict === 'Phishing' || aiData.verdict === 'BEC' ? '#3B82F6' : aiData.verdict === 'Legitimate' ? '#22C55E' : '#EAB308'}` }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 }}>
                      <div style={{ fontWeight: 700, fontSize: '1.1rem', color: aiData.verdict === 'Phishing' || aiData.verdict === 'BEC' ? '#3B82F6' : aiData.verdict === 'Legitimate' ? '#22C55E' : '#EAB308' }}>{aiData.verdict}</div>
                ...
                <div className="at-card" style={{ padding: 14 }}>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 8, textAlign: 'center' }}>
                    {[['SPF', result.spf_result], ['DKIM', result.dkim_result], ['DMARC', result.dmarc_result]].map(([name, val]) => (
                      <div key={name} style={{ padding: '10px', background: '#1F2937', borderRadius: 6 }}>
