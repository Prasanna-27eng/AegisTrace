...
const SOURCE_META = {
  cisa_kev:      { label: 'CISA KEV',        color: '#4DA3FF', icon: AlertTriangle, url: 'https://www.cisa.gov/known-exploited-vulnerabilities-catalog' },
...
function SourceBadge({ source }) {
  const m = SOURCE_META[source] || { label: source, color: '#71717A' };
  return (
    <span style={{
  ...
  return (
    <div className="at-card" style={{ padding: '12px 16px' }}>
      ...
      >
        ...
        <span style={{ fontSize: '0.68rem', color: '#71717A', fontFamily: 'JetBrains Mono' }}>
        ...
        <a href={meta.url} target="_blank" rel="noreferrer" onClick={e => e.stopPropagation()}
          style={{ marginLeft: 4, color: '#71717A' }}>
      ...
      {open && items.length === 0 && (
        <div style={{ fontSize: '0.75rem', color: '#71717A', padding: '8px 0' }}>No items returned.</div>
      )}
        ...
        <>
          ...
          {feedKey === 'cisa_kev' && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 5 }}>
              {items.map((v, i) => (
                <div key={i} style={{ display: 'flex', gap: 10, padding: '7px 10px', background: '#0D111C', borderRadius: 5, fontSize: '0.74rem', alignItems: 'flex-start' }}>
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <div style={{ color: '#71717A', fontSize: '0.68rem', lineHeight: 1.4 }}>{v.description}</div>
                    {v.action && <div style={{ color: '#EAB308', fontSize: '0.67rem', marginTop: 3 }}>→ {v.action}</div>}
                  </div>
                  <div style={{ flexShrink: 0, textAlign: 'right' }}>
                    <div style={{ fontSize: '0.62rem', color: '#71717A', fontFamily: 'JetBrains Mono' }}>Added {v.date_added}</div>
          ...
          {feedKey === 'urlhaus' && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
              {items.map((u, i) => (
                <div key={i} style={{ display: 'flex', gap: 8, padding: '6px 10px', background: '#0D111C', borderRadius: 5, fontSize: '0.72rem', alignItems: 'center' }}>
                  <span style={{
                    ...
                    color: u.url_status === 'online' ? '#EF4444' : '#71717A',
                    fontFamily: 'JetBrains Mono', fontWeight: 600,
                  }}>{u.url_status}</span>
                  <span style={{ fontFamily: 'JetBrains Mono', color: '#B0B0C0', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', flex: 1, fontSize: '0.68rem' }}>{u.url}</span>
                  {u.tags?.length > 0 && <span style={{ color: '#A78BFA', fontSize: '0.62rem', flexShrink: 0 }}>{u.tags.slice(0, 2).join(', ')}</span>}
                  <span style={{ color: '#71717A', fontSize: '0.62rem', flexShrink: 0, fontFamily: 'JetBrains Mono' }}>{timeAgo(u.date_added)}</span>
                </div>
              ))}
            </div>
          )}
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
              {items.map((t, i) => (
                <div key={i} style={{ display: 'flex', gap: 8, padding: '6px 10px', background: '#0D111C', borderRadius: 5, fontSize: '0.72rem', alignItems: 'center' }}>
                  ...
                  <span style={{ fontFamily: 'JetBrains Mono', color: '#B0B0C0', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', flex: 1, fontSize: '0.68rem' }}>{t.ioc}</span>
                  <span style={{ color: '#71717A', fontSize: '0.7rem', flexShrink: 0 }}>{t.malware || t.threat_type}</span>
                  <span style={{ color: '#71717A', fontSize: '0.62rem', flexShrink: 0, fontFamily: 'JetBrains Mono' }}>{t.confidence}%</span>
                </div>
              ))}
            </div>
          )}
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
              {items.map((s, i) => (
                <div key={i} style={{ display: 'flex', gap: 8, padding: '6px 10px', background: '#0D111C', borderRadius: 5, fontSize: '0.72rem', alignItems: 'center' }}>
                  ...
                  <span style={{ color: '#71717A', fontSize: '0.62rem', flexShrink: 0, fontFamily: 'JetBrains Mono' }}>{timeAgo(s.first_seen)}</span>
                </div>
              ))}
            </div>
          )}
...
export default function ThreatFeeds() {
  ...
  return (
    <div style={{ padding: '20px 24px', maxWidth: 1000 }}>
      ...
      {loading ? (
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 10, padding: '48px 0', color: '#71717A' }}>
          <Loader2 size={22} style={{ animation: 'spin 1s linear infinite', color: '#A78BFA' }} />
