...
const CATEGORY_META = {
  ...
  agent_action:    { label: 'Agent Action',      color: '#2563EB',  Icon: Bot },
  ...
  response_action: { label: 'Response Action',   color: '#2563EB',  Icon: Zap },
  action:          { label: 'Action',            color: 'rgba(26,22,18,0.5)',  Icon: Activity },
};
...
const TRUST_COLOR = {
  ...
  unverified: '#EAB308',
  suspicious: '#EF4444',
  revoked:    'rgba(26,22,18,0.35)',
};

const ACTOR_TYPE_COLOR = { human: 'rgba(26,22,18,0.6)', ai: 'rgba(26,22,18,0.7)', agent: '#2563EB', system: 'rgba(26,22,18,0.5)' };
...
export default function TrustTimelineTab({ caseId }) {
  ...
  return (
    <div style={{ padding: '20px 24px', maxWidth: 900 }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 20 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          ...
          <div>
            ...
            <div style={{ fontSize: '0.7rem', color: 'rgba(26,22,18,0.5)', marginTop: 1 }}>Chain of trust events — logins, token use, privilege changes, agent actions, approvals</div>
          </div>
        </div>
      ...
      {/* Add form */}
      {showAdd && (
        <div className="at-card" style={{ padding: 16, marginBottom: 18, borderColor: 'rgba(26,22,18,0.12)' }}>
            ...
            return (
              <div key={ev.id} style={{ position: 'relative', marginBottom: 18, paddingLeft: 24 }}>
                ...
                <div style={{
                  ...
                  background: TRUST_COLOR[ev.trust_level] || 'rgba(26,22,18,0.35)',
                  border: `2px solid #111111`,
                  boxShadow: `0 0 8px ${TRUST_COLOR[ev.trust_level] || 'rgba(26,22,18,0.35)'}66`,
                }} />
                ...
                <div style={{ background: 'rgba(26,22,18,0.02)', border: '1px solid rgba(26,22,18,0.07)', borderRadius: 8, padding: '12px 14px' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6 }}>
                    ...
                    <span style={{ marginLeft: 'auto', fontSize: '0.65rem', color: 'rgba(26,22,18,0.5)', ...MONO }}>{timeLabel(ev.timestamp)}</span>
                  </div>
                  <div style={{ fontSize: '0.82rem', color: 'rgba(240,240,248,0.85)', marginBottom: 6, lineHeight: 1.5 }}>{ev.description}</div>
                  <div style={{ display: 'flex', gap: 12, fontSize: '0.68rem', color: 'rgba(26,22,18,0.5)', ...MONO }}>
                    {ev.actor && <span>Actor: <span style={{ color: ACTOR_TYPE_COLOR[ev.actor_type] || 'rgba(26,22,18,0.6)' }}>{ev.actor}</span></span>}
                    {ev.actor_type && <span style={{ color: ACTOR_TYPE_COLOR[ev.actor_type] || 'rgba(26,22,18,0.5)' }}>[{ev.actor_type}]</span>}
