...
function AttackPathViz({ data }) {
  ...
  const NODE_COLORS = {
    user: '#4A7EC8', service_account: '#F59E0B', api_key: '#8B5CF6',
    ...
    <div style={{ overflowX: 'auto', overflowY: 'auto', maxHeight: 520, padding: '12px 0' }}>
      ...
      {data.summary && (
        <div style={{ padding: '10px 16px', margin: '0 16px 16px',
          background: 'rgba(74,126,200,0.06)', border: '1px solid rgba(74,126,200,0.15)',
      ...
      <svg width={Math.max(TOTAL_W, 600)} height={totalH + 20} style={{ display: 'block' }}>
        ...
        {Object.entries(byDepth).map(([depth, nodes]) => (
          <text key={depth}
            ...
            textAnchor="middle" fontSize="9" fill="rgba(148,163,184,0.5)"
          ...
          return (
            <g key={i}>
              <line x1={src.x} y1={src.y} x2={tgt.x} y2={tgt.y}
                stroke="rgba(74,126,200,0.35)" strokeWidth="1.5" strokeDasharray="4 3"/>
              {/* Arrow */}
              <polygon
        ...
        {data.nodes.map(node => {
          ...
          if (!pos) return null;
          const color = NODE_COLORS[node.node_type] || '#4A7EC8';
          ...
          return (
            <g key={node.id} transform={`translate(${pos.x - 64}, ${pos.y - 36})`}>
              ...
              <rect width={128} height={72} rx={6}
                fill={node.is_compromised ? 'rgba(239,68,68,0.08)' : 'rgba(74,126,200,0.06)'}
                stroke={node.is_compromised ? 'rgba(239,68,68,0.4)' : 'rgba(74,126,200,0.25)'}
...
export default function AttackGraphTab({ caseData, updateCase, caseId }) {
  ...
  return (
    <div style={{ padding: '20px 24px', maxWidth: 920 }}>
      ...
      {/* Tab row */}
      <div style={{ display: 'flex', gap: 2, padding: '12px 0 12px 0', borderBottom: '1px solid rgba(74,126,200,0.12)', marginBottom: 20 }}>
        {[['timeline', 'Timeline'], ['path', 'Attack Path']].map(([id, label]) => (
          <button key={id} onClick={() => { setActiveTab(id); if (id === 'path') loadAttackPath(); }}
            style={{
              padding: '7px 16px',
              background: activeTab === id ? 'rgba(74,126,200,0.12)' : 'transparent',
      ...
      {activeTab === 'path' && (
        pathLoading
          ? <div style={{ padding: 40, textAlign: 'center', color: '#787878', fontFamily: 'JetBrains Mono, monospace', fontSize: 13 }}>
