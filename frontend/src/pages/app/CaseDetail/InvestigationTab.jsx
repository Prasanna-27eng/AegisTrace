...
export default function InvestigationTab({ caseData, updateCase, caseId }) {
  ...
  const scoreColor = { confirmed: '#22C55E', strong: 'rgba(26,22,18,0.7)', weak: '#EAB308', hypothesis: 'rgba(26,22,18,0.5)' };
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20 }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          ...
          <div className="at-card" style={{ padding: 16 }}>
            ...
            {evidence.length === 0 ? (
              <div style={{ color: 'rgba(26,22,18,0.5)', fontSize: '0.8rem', textAlign: 'center', padding: '20px 0' }}>No evidence yet.</div>
            ) : (
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                {evidence.map(ev => (
                  <div key={ev.id} style={{ background: 'var(--surface)', borderRadius: 6, padding: '10px 12px', border: `1px solid ${ev.analyst_confirmed ? 'rgba(34,197,94,0.2)' : 'rgba(26,22,18,0.05)'}` }}>
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 4 }}>
                      <span style={{ fontSize: '0.72rem', fontWeight: 600, color: 'rgba(26,22,18,0.6)' }}>{ev.artifact_type}</span>
                      <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
                        ...
                        <span style={{ fontSize: '0.65rem', color: 'rgba(26,22,18,0.5)' }}>{ev.confidence}%</span>
                        <button onClick={() => confirmEvidence(ev)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: ev.analyst_confirmed ? '#22C55E' : 'rgba(26,22,18,0.5)', padding: 2 }} title={ev.analyst_confirmed ? 'Confirmed' : 'Confirm'}>
                          {ev.analyst_confirmed ? <Check size={12} /> : <Check size={12} />}
                        </button>
                      </div>
                    </div>
                    {ev.verdict && <div style={{ fontSize: '0.72rem', color: 'rgba(26,22,18,0.5)' }}>Verdict: <span style={{ color: ev.verdict === 'malicious' ? '#EF4444' : '#EAB308' }}>{ev.verdict}</span></div>}
                    {ev.normalized_output && <div style={{ fontSize: '0.72rem', color: 'rgba(26,22,18,0.5)', marginTop: 4, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{ev.normalized_output.slice(0,80)}</div>}
                    {ev.ai_analysis && (
                      <div style={{ marginTop: 6, padding: '6px 8px', background: 'rgba(143,175,192,0.06)', borderRadius: 4, borderLeft: '2px solid rgba(143,175,192,0.3)' }}>
                        ...
                        <div style={{ fontSize: '0.72rem', color: 'rgba(26,22,18,0.5)', marginTop: 2 }}>{ev.ai_analysis.slice(0, 120)}…</div>
                      </div>
                    )}
                  </div>
                ))}
          ...
          {/* Add evidence modal */}
          {showAddEv && (
            <div className="at-card" style={{ padding: 16, borderColor: 'rgba(26,22,18,0.138)' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 12 }}>
                <div className="section-label" style={{ margin: 0 }}>Add Evidence</div>
                <button onClick={() => setShowAddEv(false)} style={{ background: 'none', border: 'none', color: 'rgba(26,22,18,0.5)', cursor: 'pointer' }}><X size={14} /></button>
              </div>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
                  <div>
                    <label style={{ fontSize: '0.7rem', color: 'rgba(26,22,18,0.5)', display: 'block', marginBottom: 4 }}>Type</label>
                  ...
                  <div>
                    <label style={{ fontSize: '0.7rem', color: 'rgba(26,22,18,0.5)', display: 'block', marginBottom: 4 }}>Score</label>
                ...
                <div>
                  <label style={{ fontSize: '0.7rem', color: 'rgba(26,22,18,0.5)', display: 'block', marginBottom: 4 }}>Raw Input</label>
                ...
                <div>
                  <label style={{ fontSize: '0.7rem', color: 'rgba(26,22,18,0.5)', display: 'block', marginBottom: 4 }}>Normalized Output / Notes</label>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
                  <div>
                    <label style={{ fontSize: '0.7rem', color: 'rgba(26,22,18,0.5)', display: 'block', marginBottom: 4 }}>Verdict</label>
                  ...
                  <div>
                    <label style={{ fontSize: '0.7rem', color: 'rgba(26,22,18,0.5)', display: 'block', marginBottom: 4 }}>Confidence %</label>
