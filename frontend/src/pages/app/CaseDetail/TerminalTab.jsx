              ...
              <div className="at-card" style={{ padding: 14, borderLeft: '2px solid rgba(167,139,250,0.4)' }}>
                ...
                {result.key_findings?.length > 0 && (
                  <div style={{ marginBottom: 10 }}>
                    ...
                    {result.key_findings.map((f, i) => <div key={i} style={{ fontSize: '0.8rem', color: '#F0F0F8', padding: '3px 0', display: 'flex', gap: 8 }}><span style={{ color: '#C0392B' }}>→</span>{f}</div>)}
                  </div>
                )}
          ...
          {history.length > 0 && (
            <div className="at-card" style={{ padding: 14 }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 6, maxHeight: 200, overflowY: 'auto' }}>
                {history.map((run, i) => (
                  <div key={run.id || i} style={{ padding: '7px 10px', background: '#0F1018', borderRadius: 5, cursor: 'pointer', fontSize: '0.78rem' }}
