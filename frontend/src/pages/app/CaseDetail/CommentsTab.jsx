    ...
    <div style={{ padding: '20px 24px', maxWidth: 800 }}>
      ...
      <div className="at-card" style={{ padding: 16, marginBottom: 20 }}>
        <div style={{ display: 'flex', gap: 10, alignItems: 'flex-start' }}>
          <div style={{ width: 32, height: 32, borderRadius: '50%', background: 'rgba(99,102,241,0.15)', border: '1px solid rgba(99,102,241,0.3)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 700, color: '#6366F1', fontSize: '0.8rem', flexShrink: 0 }}>
            {user?.name?.[0] || 'A'}
          </div>
          <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 8 }}>
            ...
            <textarea
              ...
              style={{ background: '#080C14', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 6, color: '#F0F0F8', fontSize: '0.82rem', padding: '10px 12px', resize: 'vertical', outline: 'none', lineHeight: 1.6 }}
            />
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              ...
              <span style={{ fontSize: '0.68rem', color: '#71717A', ...MONO }}>Ctrl+Enter to submit</span>
            </div>
          </div>
        </div>
      </div>
    ...
    <div style={{ background: c.is_pinned ? 'rgba(255,255,255,0.025)' : 'rgba(240,240,248,0.02)', border: `1px solid ${c.is_pinned ? 'rgba(255,255,255,0.12)' : 'rgba(255,255,255,0.07)'}`, borderLeft: `3px solid ${meta.color}`, borderRadius: 8, padding: '14px 16px' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 10 }}>
        ...
        <div style={{ flex: 1 }}>
          ...
          <span style={{ fontSize: '0.7rem', color: '#71717A', marginLeft: 8, ...MONO }}>{timeAgo(c.created_at)}</span>
        </div>
        ...
        <div style={{ display: 'flex', gap: 4 }}>
          <button onClick={() => togglePin(c)} style={{ background: 'none', border: 'none', color: c.is_pinned ? '#EAB308' : '#71717A', cursor: 'pointer', padding: 3 }} title="Pin"><Pin size={11} /></button>
          <button onClick={() => setEditing({ id: c.id, body: c.body })} style={{ background: 'none', border: 'none', color: '#71717A', cursor: 'pointer', padding: 3 }}><Edit size={11} /></button>
      ...
      {isEditing ? (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
          <textarea
            ...
            rows={3}
            style={{ background: '#080C14', border: '1px solid rgba(255,255,255,0.12)', borderRadius: 6, color: '#F0F0F8', fontSize: '0.8rem', padding: '8px 10px', resize: 'vertical', outline: 'none' }}
          />
