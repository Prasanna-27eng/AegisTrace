    ...
    <div style={{ padding: '20px 24px', maxWidth: 700 }}>
      <div style={{ marginBottom: 24 }}>
        ...
        <div style={{ height: 4, background: 'rgba(255,255,255,0.08)', borderRadius: 2, overflow: 'hidden' }}>
          <div style={{ height: '100%', width: `${progress}%`, background: progress === 100 ? '#22C55E' : '#C0392B', borderRadius: 2, transition: 'width 0.3s ease' }} />
        </div>
      </div>
          ...
          {playbook.map(task => (
            <div key={task.id} onClick={() => toggle(task.id)} style={{ display: 'flex', alignItems: 'flex-start', gap: 10, padding: '8px 0', cursor: 'pointer', borderBottom: '1px solid rgba(255,255,255,0.04)' }}>
              ...
              <div>
                ...
                {task.milestone && <div style={{ fontSize: '0.65rem', color: '#C0392B', fontFamily: 'JetBrains Mono', marginTop: 2 }}>MILESTONE</div>}
              </div>
            </div>
          ))}
        </div>
