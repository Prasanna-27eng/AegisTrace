    ...
    <div style={{ padding:'24px 28px' }}>
      ...
      <div style={{ display:'flex', borderBottom:'1px solid rgba(255,255,255,0.07)', marginBottom:20 }}>
        ...
        <button className={`tab-btn ${tab==='history'?'active':''}`} onClick={() => setTab('history')}>
          History {history.length > 0 && <span style={{ marginLeft:5, background:'rgba(192,57,43,0.15)', color:'#C0392B', borderRadius:10, fontSize:'0.6rem', fontWeight:700, padding:'1px 5px', fontFamily:'JetBrains Mono' }}>{history.length}</span>}
        </button>
      </div>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:20 }}>
          ...
          <div style={{ display:'flex', flexDirection:'column', gap:12 }}>
            ...
            <div onDrop={handleDrop} onDragOver={e => e.preventDefault()}
              ...
              <div style={{ fontSize:'0.8rem', color:'#71717A', marginBottom:6 }}>Drop a log file or <label style={{ color:'#C0392B', cursor:'pointer' }}>browse<input type="file" accept=".log,.txt,.json,.csv" style={{ display:'none' }} onChange={handleFile}/></label></div>
                  ...
                  <div className="at-card" style={{ padding:14 }}>
                    ...
                    {result.key_findings.map((f,i) => (
                      <div key={i} style={{ fontSize:'0.8rem', padding:'4px 0', display:'flex', gap:8, borderBottom:'1px solid rgba(255,255,255,0.04)' }}>
                        <span style={{ color:'#C0392B', flexShrink:0 }}>→</span><span style={{ color:'#F0F0F8' }}>{f}</span>
                      </div>
                    ))}
                  </div>
                )}
                  ...
                  <div className="at-card" style={{ padding:14 }}>
                    ...
                    <div style={{ display:'flex', flexDirection:'column', gap:6, maxHeight:180, overflowY:'auto' }}>
                      {result.suspicious_entries.map((e,i) => (
                        <div key={i} style={{ background:'#0F1018', borderRadius:5, padding:'7px 10px', borderLeft:`2px solid ${SEVERITY_COLOR[e.severity]||'#71717A'}` }}>
          ...
          {history.length === 0 ? (
            ...
            <div key={h.id} className="at-card" style={{ padding:'12px 16px', cursor:'pointer', transition:'border-color 0.15s' }}
              ...
              <div style={{ display:'flex', alignItems:'center', gap:12 }}>
                ...
                {h.case_id && <span style={{ fontSize:'0.65rem', background:'rgba(192,57,43,0.1)', border:'1px solid rgba(192,57,43,0.25)', color:'#C0392B', padding:'2px 6px', borderRadius:3, fontFamily:'JetBrains Mono' }}>CASE</span>}
              </div>
            </div>
          ))}
        </div>
