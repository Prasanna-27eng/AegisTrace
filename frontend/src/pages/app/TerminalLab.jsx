...
const TYPE_COLOR = {
  ...
  url: '#22C55E', email: '#3B82F6', port: '#71717A',
};
        ...
        {sessions.map(s => (
          <div
            ...
            style={{
              ...
              border: `1px solid ${activeId === s.id ? 'rgba(59,130,246,0.3)' : 'transparent'}`,
        ...
        {showParsed && (
          <div style={{ flex: '0 0 50%', padding: 14, overflow: 'auto', display: 'flex', flexDirection: 'column', gap: 14 }}>
            ...
            {parsed.mitre_techniques?.length > 0 && (
              <div>
                <div style={{ fontSize: '0.62rem', color: '#3B82F6', textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: 6, ...MONO }}>MITRE ATT&CK</div>
                {parsed.mitre_techniques.map(t => (
                  <div key={t.id} style={{ display: 'flex', gap: 8, padding: '4px 0', fontSize: '0.72rem' }}>
                    <span style={{ color: '#3B82F6', ...MONO, flexShrink: 0 }}>{t.id}</span>
            ...
            {iocs.length > 0 && (
              <div>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
                  {iocs.map((ioc, i) => (
                    <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '4px 8px', background: '#111827', borderRadius: 4, borderLeft: `2px solid ${TYPE_COLOR[ioc.type] || '#71717A'}` }}>
  ...
  return (
    <div style={{ display: 'flex', height: '100%', background: '#111827', overflow: 'hidden' }}>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
        ...
        {/* Header */}
        <div style={{ padding: '10px 16px', borderBottom: '1px solid rgba(255,255,255,0.07)', display: 'flex', alignItems: 'center', gap: 12, flexShrink: 0, background: '#111827' }}>
        ...
        {!activeId ? (
          ...
          <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
            ...
            <div style={{ flex: 1, overflow: 'auto', padding: '10px 16px', background: '#080A10' }} ref={outputRef}>
              {commands.length === 0 ? (
                <div style={{ padding: '24px 0', color: '#71717A', fontSize: '0.78rem', ...MONO }}>
                  <span style={{ color: '#3B82F6' }}>aegistrace@lab:~$</span> _
                ...
                commands.map(cmd => (
                  <div
                    ...
                    style={{
                      ...
                      border: `1px solid ${activeCmd?.id === cmd.id ? 'rgba(59,130,246,0.3)' : 'rgba(255,255,255,0.04)'}`,
                      background: activeCmd?.id === cmd.id ? 'rgba(59,130,246,0.05)' : 'transparent',
                      transition: 'all 0.1s',
                    }}
                  >
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4 }}>
                      <span style={{ color: '#3B82F6', ...MONO, fontSize: '0.72rem' }}>aegistrace@lab:~$</span>
            ...
            <div style={{ padding: '8px 14px', borderTop: '1px solid rgba(255,255,255,0.05)', background: '#09090F', display: 'flex', gap: 5, flexWrap: 'wrap', flexShrink: 0 }}>
              {TOOL_PRESETS.map(p => (
                <button
                  ...
                  style={{ padding: '3px 8px', borderRadius: 3, fontSize: '0.65rem', ...MONO, border: '1px solid rgba(255,255,255,0.08)', background: tool === p.name ? 'rgba(59,130,246,0.12)' : 'transparent', color: tool === p.name ? '#3B82F6' : 'rgba(240,240,248,0.4)', cursor: 'pointer' }}
                >
                  {p.name}
                </button>
              ))}
            ...
            {/* Command input row */}
            <div style={{ padding: '10px 14px', borderTop: '1px solid rgba(255,255,255,0.07)', background: '#111827', flexShrink: 0 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: showPaste ? 8 : 0 }}>
                <span style={{ color: '#3B82F6', ...MONO, fontSize: '0.8rem', flexShrink: 0 }}>aegistrace@lab:~$</span>
                <input
                  ref={inputRef}
                  value={command}
