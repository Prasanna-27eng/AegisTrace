...
function ResultRenderer({ toolKey, result }) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      ...
      <div style={{ background: 'var(--surface)', border: '1px solid rgba(26,22,18,0.08)', borderRadius: 8, padding: 16, fontFamily: 'JetBrains Mono', fontSize: '0.75rem', color: '#A8A8A8', overflow: 'auto', maxHeight: 600, whiteSpace: 'pre-wrap', wordBreak: 'break-all', lineHeight: 1.7 }}>
...
export default function ToolResult() {
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'var(--bg)', color: 'var(--text-primary)' }}>
      {/* Header */}
      <header style={{ background: 'rgba(26,22,18,0.12)', borderBottom: '1px solid rgba(26,22,18,0.08)', padding: '14px 32px', display: 'flex', alignItems: 'center', gap: 20, position: 'sticky', top: 0, zIndex: 10 }}>
        ...
        {run && <span style={{ fontFamily: 'JetBrains Mono', fontSize: '0.68rem', color: 'rgba(26,22,18,0.35)', marginLeft: 4 }}>Run #{runId}</span>}
        <div style={{ marginLeft: 'auto', display: 'flex', gap: 10, alignItems: 'center' }}>
          {run && <span style={{ fontFamily: 'JetBrains Mono', fontSize: '0.7rem', color: 'rgba(26,22,18,0.35)' }}>{new Date(run.executed_at).toLocaleString()}</span>}
          <button onClick={() => navigate('/app/hardware/tools')} style={{ display: 'flex', alignItems: 'center', gap: 6, background: 'rgba(26,22,18,0.07)', border: '1px solid rgba(26,22,18,0.1)', borderRadius: 6, color: 'var(--text-primary)', padding: '7px 14px', cursor: 'pointer', fontSize: '0.78rem', fontFamily: 'JetBrains Mono' }}>
