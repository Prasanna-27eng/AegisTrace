...
import { ExternalLink, Copy, Search } from '../../components/icons';
import api from '../../api/client';
import useStore from '../../store/useStore';

const TOOLS = [
  { id: 'whois', name: 'WHOIS / RDAP Lookup', desc: 'Domain registration and ownership lookup', inputs: ['domain'], link: 'https://lookup.icann.org', linkLabel: 'ICANN RDAP', color: 'var(--accent)' },
  { id: 'dns', name: 'DNS Lookup', desc: 'Query DNS records — A, MX, TXT, NS, CNAME', inputs: ['domain'], link: 'https://toolbox.googleapps.com/apps/dig/', linkLabel: 'Google Dig', color: 'rgba(26,22,18,0.7)' },
  ...
  { id: 'urlscan', name: 'URL Scanner', desc: 'Submit URL for sandboxed scanning', inputs: ['url'], link: 'https://urlscan.io/', linkLabel: 'urlscan.io', color: 'var(--accent)' },
  { id: 'otx', name: 'AlienVault OTX', desc: 'Open threat intelligence platform', inputs: ['ip','domain','hash'], link: 'https://otxr.alienvault.com', linkLabel: 'OTX Pulse', color: 'rgba(26,22,18,0.7)' },
  ...
  { id: 'mxtoolbox', name: 'MX Toolbox', desc: 'Email delivery and blacklist checks', inputs: ['domain','ip'], link: 'https://mxtoolbox.com/', linkLabel: 'MX Toolbox', color: 'var(--accent)' },
  { id: 'vt-link', name: 'VirusTotal Search', desc: 'Search VirusTotal for any IOC', inputs: ['ip','domain','hash','url'], link: 'https://www.virustotal.com/', linkLabel: 'VirusTotal', color: 'rgba(26,22,18,0.7)' },
  ...
  return (
    <div style={{ padding: '24px 28px' }}>
      <div style={{ marginBottom: 20 }}>
        ...
        <div style={{ fontSize: '0.75rem', color: 'var(--text-muted)', marginTop: 2 }}>Free tools and public intelligence sources</div>
      </div>
        ...
        {TOOLS.map(tool => (
          ...
          >
            <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 8 }}>
              <div style={{ fontWeight: 600, fontSize: '0.85rem', color: 'var(--text-primary)' }}>{tool.name}</div>
              <div style={{ width: 8, height: 8, borderRadius: '50%', background: tool.color, flexShrink: 0, marginTop: 3 }} />
            </div>
            <div style={{ fontSize: '0.78rem', color: 'var(--text-muted)', marginBottom: 10, lineHeight: 1.5 }}>{tool.desc}</div>
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 4, marginBottom: 10 }}>
              {tool.inputs.map(inp => <span key={inp} style={{ fontSize: '0.65rem', background: 'rgba(26,22,18,0.05)', border: '1px solid rgba(26,22,18,0.09)', color: 'var(--text-muted)', padding: '2px 6px', borderRadius: 3, fontFamily: 'JetBrains Mono' }}>{inp}</span>)}
            </div>
            ...
            {tool.id === 'ioc-extract' && activeInternal === tool.id && (
              <div style={{ marginTop: 12, borderTop: '1px solid rgba(26,22,18,0.08)', paddingTop: 12 }} onClick={e => e.stopPropagation()}>
                ...
                {iocResult && (
                  <div>
                    <div style={{ fontSize: '0.72rem', color: 'var(--text-muted)', marginBottom: 6 }}>{iocResult.count} IOCs found</div>
