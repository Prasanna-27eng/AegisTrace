...
const TACTIC_COLOR = {
  'Initial Access': '#C0392B', 'Execution': '#EF4444', 'Persistence': '#EAB308',
      ...
      {!loading && (
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(140px,1fr))', gap: 10, marginBottom: 20 }}>
          {[
            ...
            { label: 'Campaign IOCs',       val: stats.campaign_iocs,      color: '#C0392B' },
        ...
        {TABS.map(t => (
          <button key={t.id} className={`tab-btn ${tab === t.id ? 'active' : ''}`} onClick={() => setTab(t.id)}>
            ...
            {t.id === 'campaigns' && campaigns.length > 0 && (
              <span style={{ marginLeft: 6, background: 'rgba(192,57,43,0.2)', color: '#C0392B', borderRadius: 10, fontSize: '0.6rem', fontWeight: 700, padding: '1px 6px', fontFamily: 'JetBrains Mono' }}>{campaigns.length}</span>
            )}
          </button>
        ))}
      </div>
                  ...
                  {iocs.map(ioc => (
                    <div key={ioc.id} className="at-card" style={{ padding: '12px 16px', borderLeft: ioc.is_campaign ? '2px solid #C0392B' : '1px solid rgba(255,255,255,0.07)' }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                        ...
                        {ioc.is_campaign && (
                          <span style={{ fontSize: '0.65rem', color: '#C0392B', fontFamily: 'JetBrains Mono', background: 'rgba(192,57,43,0.1)', border: '1px solid rgba(192,57,43,0.3)', padding: '2px 6px', borderRadius: 3 }}>⚠ CAMPAIGN</span>
                        )}
                        <span style={{ fontSize: '0.78rem', fontWeight: 700, color: ioc.case_count > 1 ? '#C0392B' : '#71717A' }}>{ioc.case_count} case{ioc.case_count !== 1 ? 's' : ''}</span>
                      </div>
                      {ioc.cases?.length > 0 && (
                    ...
                    return (
                      <div key={m.technique_id} className="at-card" style={{ padding: '14px 16px' }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 8 }}>
                          <span style={{ fontSize: '0.75rem', color: '#A78BFA', fontFamily: 'JetBrains Mono', minWidth: 80 }}>{m.technique_id}</span>
            ...
            <div>
              {campaigns.length === 0 ? (
                <div className="at-card" style={{ padding: 40, textAlign: 'center', color: '#71717A' }}>
                  <AlertTriangle size={28} style={{ color: 'rgba(192,57,43,0.3)', margin: '0 auto 10px' }} />
                  ...
                  {campaigns.map((camp, i) => (
                    <div key={i} className="at-card" style={{ padding: '16px 18px', borderLeft: '2px solid #C0392B' }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 10 }}>
                        ...
                        <span style={{ fontSize: '0.65rem', background: 'rgba(192,57,43,0.12)', border: '1px solid rgba(192,57,43,0.3)', color: '#C0392B', padding: '2px 8px', borderRadius: 4, fontFamily: 'JetBrains Mono' }}>
                      ...
                      <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                        {searchResults.ioc_matches.map((ioc, i) => (
                          <div key={i} className="at-card" style={{ padding: '10px 14px', display: 'flex', gap: 12, alignItems: 'center' }}>
                            ...
                            <span style={{ fontSize: '0.65rem', color: '#71717A', fontFamily: 'JetBrains Mono', background: 'rgba(255,255,255,0.05)', padding: '2px 6px', borderRadius: 3 }}>{ioc.ioc_type}</span>
                            <span style={{ fontSize: '0.75rem', fontWeight: 600, color: ioc.case_count > 1 ? '#C0392B' : '#71717A' }}>{ioc.case_count} case{ioc.case_count !== 1 ? 's' : ''}</span>
                          </div>
                        ))}
                      </div>
                    </div>
