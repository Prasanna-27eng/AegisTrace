...
function AttentionRow({ c, navigate }) {
  ...
  return (
    <div
      ...
      style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '9px 12px', borderRadius: 6, cursor: 'pointer', background: '#0D1117', transition: 'background 0.15s', position: 'relative', overflow: 'hidden' }}
      onMouseEnter={e => e.currentTarget.style.background = 'rgba(255,255,255,0.04)'}
      onMouseLeave={e => e.currentTarget.style.background = '#0D1117'}
    >
      {c.severity === 'critical' && <div style={{ position: 'absolute', left: 0, top: 0, bottom: 0, width: 2, background: '#6366F1' }} />}
...
function StatTile({ icon: Icon, label, value, sub, subColor, color, onClick }) {
  return (
    <div
      ...
      onMouseEnter={e => onClick && (e.currentTarget.style.borderColor = 'rgba(99,102,241,0.3)')}
...
export default function Dashboard() {
  ...
  return (
    <div style={{ padding: '20px 24px', maxWidth: 1400 }}>
      ...
      {/* ── ATTENTION ZONE ── */}
      {attention.length > 0 && (
        <div style={{ marginBottom: 20, padding: '14px 16px', background: 'rgba(99,102,241,0.04)', border: '1px solid rgba(99,102,241,0.15)', borderRadius: 8 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 7, marginBottom: 12 }}>
            <Zap size={13} style={{ color: '#6366F1' }} />
            <span style={{ fontSize: '0.7rem', fontWeight: 600, color: '#6366F1', textTransform: 'uppercase', letterSpacing: '0.08em', fontFamily: 'JetBrains Mono' }}>
              Needs attention now
            </span>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(130px, 1fr))', gap: 10, marginBottom: 20 }}>
        <StatTile icon={AlertTriangle} label="Critical Open"   value={critical.length}     color="#6366F1"  sub={critical.length ? 'Act now' : 'None active'} subColor={critical.length ? '#EF4444' : '#22C55E'} onClick={() => navigate('/app/cases?severity=critical&status=open')} />
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1.5fr 1fr', gap: 16 }}>
        ...
        <div className="at-card" style={{ padding: '14px 16px' }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 14 }}>
            ...
            <button onClick={() => navigate('/app/cases')} style={{ fontSize: '0.7rem', color: '#6366F1', background: 'none', border: 'none', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 4 }}>
          ...
          {recentCases.length === 0 ? (
            <div style={{ padding: '32px 0', textAlign: 'center' }}>
              <FolderOpen size={28} style={{ color: 'rgba(99,102,241,0.2)', margin: '0 auto 10px' }} />
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 5 }}>
              {recentCases.map(c => (
                <div
                  key={c.id}
                  onClick={() => navigate(`/app/cases/${c.id}`)}
                  style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '9px 12px', background: '#0D1117', borderRadius: 6, cursor: 'pointer', transition: 'background 0.15s', position: 'relative', overflow: 'hidden' }}
                  onMouseEnter={e => e.currentTarget.style.background = 'rgba(255,255,255,0.04)'}
                  onMouseLeave={e => e.currentTarget.style.background = '#0D1117'}
                >
                  {c.severity === 'critical' && <div style={{ position: 'absolute', left: 0, top: 0, bottom: 0, width: 2, background: '#6366F1' }} />}
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          ...
          <div className="at-card" style={{ padding: '14px 16px' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 6 }}>
              {[
                { label: 'New Case',       path: '/app/cases',         color: '#6366F1' },
                ...
                { label: 'Threat Hunt',    path: '/app/hunt',           color: '#6366F1' },
