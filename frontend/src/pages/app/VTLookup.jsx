  ...
  return (
    <div style={{ padding: '24px 28px' }}>
      <div style={{ marginBottom: 20 }}>
        ...
        <div style={{ fontSize: '0.75rem', color: '#71717A', marginTop: 2 }}>IP · Domain · Hash · URL</div>
      </div>
          ...
          {result && (
            <div className="at-card" style={{ padding: 20, maxWidth: 700, borderColor: verdictBorder[result.verdict] || verdictBorder.unknown, background: verdictBg[result.verdict] }}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 }}>
                <div>
                  <div style={{ fontFamily: 'JetBrains Mono', fontSize: '0.9rem', color: '#F0F0F8', marginBottom: 6 }}>{result.ioc}</div>
                  <div style={{ fontSize: '0.72rem', color: '#71717A' }}>{result.ioc_type?.toUpperCase()}</div>
                </div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 12, marginBottom: 16 }}>
                <div style={{ textAlign: 'center', padding: '12px 8px', background: 'rgba(255,255,255,0.03)', borderRadius: 6 }}>
                  ...
                  <div style={{ fontSize: '0.68rem', color: '#71717A', marginTop: 2 }}>Malicious</div>
                </div>
                <div style={{ textAlign: 'center', padding: '12px 8px', background: 'rgba(255,255,255,0.03)', borderRadius: 6 }}>
                  <div style={{ fontSize: '1.6rem', fontWeight: 700, color: '#EAB308' }}>{result.suspicious_count || 0}</div>
                  <div style={{ fontSize: '0.68rem', color: '#71717A', marginTop: 2 }}>Suspicious</div>
                </div>
                <div style={{ textAlign: 'center', padding: '12px 8px', background: 'rgba(255,255,255,0.03)', borderRadius: 6 }}>
                  <div style={{ fontSize: '1.6rem', fontWeight: 700, color: '#F0F0F8' }}>{result.total_engines}</div>
                  <div style={{ fontSize: '0.68rem', color: '#71717A', marginTop: 2 }}>Total Engines</div>
                </div>
              </div>

              {result.tags?.length > 0 && (
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: 5, marginBottom: 12 }}>
                  {result.tags.map(t => <span key={t} style={{ fontSize: '0.7rem', background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.1)', color: '#71717A', padding: '2px 8px', borderRadius: 4 }}>{t}</span>)}
                </div>
              )}

              {result.as_owner && <div style={{ fontSize: '0.78rem', color: '#71717A', marginBottom: 4 }}>AS: {result.as_owner}</div>}
              {result.country && <div style={{ fontSize: '0.78rem', color: '#71717A', marginBottom: 12 }}>Country: {result.country}</div>}
              ...
              {Object.keys(result.engines || {}).length > 0 && (
                <div style={{ marginTop: 12, borderTop: '1px solid rgba(255,255,255,0.07)', paddingTop: 12 }}>
                  <div style={{ fontSize: '0.72rem', color: '#71717A', marginBottom: 8 }}>Flagging Engines</div>
      ...
      {tab === 'history' && (
        <div>
          ...
          {history.length === 0 ? (
            <div style={{ textAlign: 'center', padding: 40, color: '#71717A', fontSize: '0.85rem' }}>No VT lookups yet.</div>
          ) : (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
              {history.map(h => (
                <div key={h.id} className="at-card" style={{ padding: '10px 14px', display: 'flex', alignItems: 'center', gap: 12 }}>
                  <span style={{ fontSize: '0.65rem', color: '#71717A', fontFamily: 'JetBrains Mono', flexShrink: 0 }}>{h.ioc_type}</span>
                  <span style={{ fontFamily: 'JetBrains Mono', fontSize: '0.8rem', flex: 1, overflow: 'hidden', textOverflow: 'ellipsis', color: '#F0F0F8' }}>{h.ioc}</span>
                  <span style={{ fontSize: '0.72rem', color: '#71717A', flexShrink: 0 }}>{h.malicious_count}/{h.total_engines}</span>
                  <VerdictBadge verdict={h.verdict} />
                  <span style={{ fontSize: '0.7rem', color: '#71717A', flexShrink: 0 }}>{new Date(h.looked_up_at).toLocaleDateString()}</span>
      ...
      {tab === 'bulk' && (
        <div style={{ maxWidth: 700 }}>
          <div style={{ marginBottom: 12 }}>
            <label style={{ fontSize: '0.75rem', color: '#71717A', display: 'block', marginBottom: 6 }}>Paste IOCs (one per line or mixed text)</label>
              ...
              {bulkResults.map((r, i) => (
                <div key={i} className="at-card" style={{ padding: '10px 14px', display: 'flex', alignItems: 'center', gap: 10 }}>
                  ...
                  {r.error ? <span style={{ fontSize: '0.72rem', color: '#EF4444' }}>Error</span> : (
                    <>
                      <span style={{ fontSize: '0.72rem', color: '#71717A' }}>{r.malicious_count}/{r.total_engines}</span>
