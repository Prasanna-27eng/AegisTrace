...
export default function VTLookup() {
  ...
  const verdictBorder = { malicious: 'rgba(239,68,68,0.2)', suspicious: 'rgba(234,179,8,0.2)', clean: 'rgba(34,197,94,0.2)', unknown: 'rgba(26,22,18,0.08)' };

  return (
    <div style={{ padding: '24px 28px' }}>
      <div style={{ marginBottom: 20 }}>
        <h1 style={{ fontSize: '1.3rem', fontWeight: 600 }}>VirusTotal Lookup</h1>
        <div style={{ fontSize: '0.75rem', color: 'var(--text-muted)', marginTop: 2 }}>IP · Domain · Hash · URL</div>
      </div>
          ...
          {result && (
            <div className="at-card" style={{ padding: 20, maxWidth: 700, borderColor: verdictBorder[result.verdict] || verdictBorder.unknown, background: verdictBg[result.verdict] }}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 }}>
                <div>
                  <div style={{ fontFamily: 'JetBrains Mono', fontSize: '0.9rem', color: 'var(--text-primary)', marginBottom: 6 }}>{result.ioc}</div>
                  <div style={{ fontSize: '0.72rem', color: 'var(--text-muted)' }}>{result.ioc_type?.toUpperCase()}</div>
                </div>
                <VerdictBadge verdict={result.verdict} />
              </div>

              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 12, marginBottom: 16 }}>
                <div style={{ textAlign: 'center', padding: '12px 8px', background: 'rgba(26,22,18,0.04)', borderRadius: 6 }}>
                  <div style={{ fontSize: '1.6rem', fontWeight: 700, color: result.malicious_count > 0 ? '#EF4444' : '#22C55E' }}>{result.malicious_count}</div>
                  <div style={{ fontSize: '0.68rem', color: 'var(--text-muted)', marginTop: 2 }}>Malicious</div>
                </div>
                <div style={{ textAlign: 'center', padding: '12px 8px', background: 'rgba(26,22,18,0.04)', borderRadius: 6 }}>
                  <div style={{ fontSize: '1.6rem', fontWeight: 700, color: '#EAB308' }}>{result.suspicious_count || 0}</div>
                  <div style={{ fontSize: '0.68rem', color: 'var(--text-muted)', marginTop: 2 }}>Suspicious</div>
                </div>
                <div style={{ textAlign: 'center', padding: '12px 8px', background: 'rgba(26,22,18,0.04)', borderRadius: 6 }}>
                  <div style={{ fontSize: '1.6rem', fontWeight: 700, color: 'var(--text-primary)' }}>{result.total_engines}</div>
                  <div style={{ fontSize: '0.68rem', color: 'var(--text-muted)', marginTop: 2 }}>Total Engines</div>
                </div>
              </div>

              {result.tags?.length > 0 && (
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: 5, marginBottom: 12 }}>
                  {result.tags.map(t => <span key={t} style={{ fontSize: '0.7rem', background: 'rgba(26,22,18,0.07)', border: '1px solid rgba(26,22,18,0.1)', color: 'var(--text-muted)', padding: '2px 8px', borderRadius: 4 }}>{t}</span>)}
                </div>
              )}

              {result.as_owner && <div style={{ fontSize: '0.78rem', color: 'var(--text-muted)', marginBottom: 4 }}>AS: {result.as_owner}</div>}
              {result.country && <div style={{ fontSize: '0.78rem', color: 'var(--text-muted)', marginBottom: 12 }}>Country: {result.country}</div>}
              ...
              {Object.keys(result.engines || {}).length > 0 && (
                <div style={{ marginTop: 12, borderTop: '1px solid rgba(26,22,18,0.08)', paddingTop: 12 }}>
                  <div style={{ fontSize: '0.72rem', color: 'var(--text-muted)', marginBottom: 8 }}>Flagging Engines</div>
      ...
      {tab === 'history' && (
        <div>
          ...
          {history.length === 0 ? (
            <div style={{ textAlign: 'center', padding: 40, color: 'var(--text-muted)', fontSize: '0.85rem' }}>No VT lookups yet.</div>
          ) : (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
              {history.map(h => (
                <div key={h.id} className="at-card" style={{ padding: '10px 14px', display: 'flex', alignItems: 'center', gap: 12 }}>
                  <span style={{ fontSize: '0.65rem', color: 'var(--text-muted)', fontFamily: 'JetBrains Mono', flexShrink: 0 }}>{h.ioc_type}</span>
                  <span style={{ fontFamily: 'JetBrains Mono', fontSize: '0.8rem', flex: 1, overflow: 'hidden', textOverflow: 'ellipsis', color: 'var(--text-primary)' }}>{h.ioc}</span>
                  <span style={{ fontSize: '0.72rem', color: 'var(--text-muted)', flexShrink: 0 }}>{h.malicious_count}/{h.total_engines}</span>
                  <VerdictBadge verdict={h.verdict} />
                  <span style={{ fontSize: '0.7rem', color: 'var(--text-muted)', flexShrink: 0 }}>{new Date(h.looked_up_at).toLocaleDateString()}</span>
      ...
      {tab === 'bulk' && (
        <div style={{ maxWidth: 700 }}>
          <div style={{ marginBottom: 12 }}>
            <label style={{ fontSize: '0.75rem', color: 'var(--text-muted)', display: 'block', marginBottom: 6 }}>Paste IOCs (one per line or mixed text)</label>
              ...
              {bulkResults.map((r, i) => (
                <div key={i} className="at-card" style={{ padding: '10px 14px', display: 'flex', alignItems: 'center', gap: 10 }}>
                  ...
                  {r.error ? <span style={{ fontSize: '0.72rem', color: '#EF4444' }}>Error</span> : (
                    <>
                      <span style={{ fontSize: '0.72rem', color: 'var(--text-muted)' }}>{r.malicious_count}/{r.total_engines}</span>
