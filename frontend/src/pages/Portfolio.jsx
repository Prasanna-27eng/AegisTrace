  ...
  useEffect(() => {
    ...
    const draw = still => {
      ...
      embers.forEach(e => {
        ...
        ctx.fillStyle = e.gold ? `rgba(204,120,92,${a * 1.3})` : `rgba(240,235,227,${a})`;
...
function HeroScene() {
  ...
  return (
    <PinnedScene vh="360vh" sceneRef={ref}>
      ...
      <motion.div aria-hidden style={{
        position: 'absolute', inset: 0,
        background: 'radial-gradient(ellipse 80% 60% at 50% 100%, rgba(204,120,92,0.15) 0%, transparent 70%)',
        x: useTransform(bgDriftX, v => v * 0.4),
        y: useTransform(bgDriftY, v => v * 0.4),
      }}/>
      <div aria-hidden style={{ position: 'absolute', inset: 0, background: 'radial-gradient(ellipse 50% 40% at 50% 105%, rgba(204,120,92,0.12) 0%, transparent 60%)' }}/>
      ...
      <motion.div style={{
        ...
        <div style={{ textAlign: 'center' }}>
          <div className="mono" style={{ fontSize: 10, letterSpacing: '0.3em', color: 'rgba(240,235,227,0.4)', marginBottom: 8 }}>CREDENTIALS</div>
        ...
        {BADGES.map((b, i) => (
          <motion.div key={b.label}
            ...
            style={{
              ...
              background: 'rgba(204,120,92,0.07)', border: '1px solid rgba(204,120,92,0.2)',
            ...
            <div className="cg" style={{ fontSize: 10, color: 'rgba(240,235,227,0.4)', marginTop: 2 }}>{b.sub}</div>
          </motion.div>
        ))}
      </motion.div>
      ...
      <motion.div style={{
        ...
        <p className="cg" style={{ fontSize: 17, fontWeight: 500, color: 'rgba(240,235,227,0.56)', maxWidth: 460, textAlign: 'center', margin: 0 }}>
...
function FlythroughStat({ p, stat, depth }) {
  ...
  return (
    <motion.div style={{
      ...
      <div className="cg" style={{ fontSize: 'clamp(13px,1.4vw,18px)', color: 'rgba(240,235,227,0.55)', marginTop: 12 }}>
        {stat.label}
      </div>
    </motion.div>
  );
...
function FlythroughScene() {
  ...
  return (
    <PinnedScene vh="360vh" sceneRef={ref}>
      ...
      {/* Gold bloom glow */}
      <div aria-hidden style={{ position: 'absolute', inset: 0, background: 'radial-gradient(ellipse 50% 50% at 50% 50%, rgba(204,120,92,0.06) 0%, transparent 70%)', pointerEvents: 'none' }}/>

      {/* Section label */}
      <motion.div style={{ position: 'absolute', top: '7vh', left: 0, right: 0, display: 'flex', justifyContent: 'center', opacity: labelOp }}>
        <span className="mono" style={{ fontSize: 10, letterSpacing: '0.28em', color: 'rgba(240,235,227,0.3)' }}>BY THE NUMBERS</span>
      </motion.div>
...
function ToolRackCard({ tool, p }) {
  ...
  return (
    <motion.div style={{ opacity: op, filter, willChange: 'filter, opacity', position: 'relative' }}>
      ...
      <motion.div aria-hidden style={{
        position: 'absolute', inset: -1, border: '1px solid rgba(204,120,92,0.6)',
      ...
      <div style={{
        ...
        <div className="mono" style={{ fontSize: 10, color: 'rgba(240,235,227,0.36)', marginBottom: 14, letterSpacing: '0.16em' }}>{tool.layer}</div>
        <div className="mono" style={{ fontSize: 'clamp(18px,2vw,26px)', fontWeight: 700, color: INK, marginBottom: 10 }}>{tool.name}</div>
        <p className="cg" style={{ fontSize: 13.5, lineHeight: 1.62, color: 'rgba(240,235,227,0.52)', margin: '0 0 18px' }}>{tool.desc}</p>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap' }}>
          <code className="mono" style={{ fontSize: 11.5, color: GOLD, background: 'rgba(204,120,92,0.08)', padding: '5px 10px', flex: 1, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{tool.cmd}</code>
          <button onClick={copy} className="cg" style={{
            background: 'none', border: '1px solid rgba(240,235,227,0.15)', color: 'rgba(240,235,227,0.5)',
            fontSize: 11, padding: '5px 10px', cursor: 'pointer', flexShrink: 0,
            transition: 'border-color 140ms, color 140ms',
          }}>{copied ? '✓' : 'copy'}</button>
          <a href={tool.href} target="_blank" rel="noopener noreferrer" className="mono" style={{ fontSize: 11, color: 'rgba(240,235,227,0.35)', textDecoration: 'none' }}>
            PyPI ↗
          </a>
        </div>
      </div>
  ...
  return (
    <PinnedScene vh="340vh" sceneRef={ref}>
      ...
      <motion.div style={{
        ...
        <div>
          <div className="mono" style={{ fontSize: 10, letterSpacing: '0.26em', color: 'rgba(240,235,227,0.3)', marginBottom: 4 }}>THE EXPANSION PACK</div>
...
function FlagshipSection() {
  ...
  return (
    <section ref={ref} style={{ padding: 'clamp(80px,10vw,130px) clamp(24px,5vw,72px)', position: 'relative', overflow: 'hidden', borderTop: '1px solid rgba(240,235,227,0.06)' }}>
      {/* Parallax depth layers */}
      <motion.div aria-hidden style={{
        position: 'absolute', inset: '-10%',
        background: 'radial-gradient(ellipse 60% 50% at 80% 50%, rgba(204,120,92,0.04) 0%, transparent 70%)',
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(320px,1fr))', gap: 'clamp(40px,6vw,80px)', alignItems: 'start' }}>
          <Reveal>
            <div>
              ...
              <p className="cg" style={{ fontSize: 16, lineHeight: 1.68, color: 'rgba(240,235,227,0.58)', maxWidth: 480, margin: '0 0 18px' }}>
              ...
              <p className="cg" style={{ fontSize: 14, lineHeight: 1.6, color: 'rgba(240,235,227,0.36)', maxWidth: 480, margin: '0 0 28px' }}>
          ...
          <div style={{ background: 'rgba(240,235,227,0.02)', border: '1px solid rgba(240,235,227,0.08)', padding: 'clamp(20px,3vw,32px)' }}>
            <div className="mono" style={{ fontSize: 10, letterSpacing: '0.2em', color: 'rgba(240,235,227,0.3)', marginBottom: 18 }}>WHAT IT SHIPS</div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              {FLAGSHIP_POINTS.map((pt, i) => (
                <Reveal key={pt} delay={i * 0.05} y={10}>
                  <div className="cg" style={{ display: 'flex', gap: 10, fontSize: 14, lineHeight: 1.55, color: 'rgba(240,235,227,0.62)' }}>
...
function BackgroundSection() {
  ...
  return (
    <section ref={ref} style={{ padding: 'clamp(80px,10vw,130px) clamp(24px,5vw,72px)', position: 'relative', borderTop: '1px solid rgba(240,235,227,0.06)' }}>
      <div style={{ maxWidth: 1240, margin: '0 auto' }}>
        {/* Animated horizontal rule */}
        <div style={{ position: 'relative', height: 1, background: 'rgba(240,235,227,0.06)', marginBottom: 'clamp(40px,5vw,64px)' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'minmax(160px,240px) 1fr', gap: 'clamp(32px,5vw,64px)' }}>
          <Reveal>
            <div className="mono" style={{ fontSize: 10, letterSpacing: '0.22em', color: 'rgba(240,235,227,0.36)' }}>BACKGROUND</div>
          </Reveal>
          <div>
            <Reveal>
            ...
            <Reveal delay={0.06}>
              <p className="cg" style={{ fontSize: 15.5, lineHeight: 1.68, color: 'rgba(240,235,227,0.52)', maxWidth: 680, margin: '0 0 28px' }}>
            ...
            <Reveal delay={0.1}>
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px,1fr))', gap: 20 }}>
                <div>
                  <div className="mono" style={{ fontSize: 10, letterSpacing: '0.2em', color: 'rgba(240,235,227,0.3)', marginBottom: 12 }}>CERTIFICATIONS</div>
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
                    {CERTS_EARNED.map(c => (
                      <span key={c} className="mono" style={{ fontSize: 12, border: '1px solid rgba(204,120,92,0.35)', color: INK, borderRadius: 100, padding: '7px 14px' }}>{c}</span>
                    ))}
                    {CERTS_PENDING.map(c => (
                      <span key={c} className="mono" style={{ fontSize: 12, border: '1px solid rgba(240,235,227,0.14)', color: 'rgba(240,235,227,0.42)', borderRadius: 100, padding: '7px 14px' }}>{c}</span>
                    ))}
                  </div>
                </div>
                <div>
                  <div className="mono" style={{ fontSize: 10, letterSpacing: '0.2em', color: 'rgba(240,235,227,0.3)', marginBottom: 12 }}>EDUCATION</div>
                  <div className="cd" style={{ fontSize: 18, fontWeight: 600, color: INK, marginBottom: 4 }}>MSc Information Systems</div>
                  <div className="cg" style={{ fontSize: 14, color: 'rgba(240,235,227,0.44)' }}>Dublin Business School · 2025</div>
                </div>
              </div>
            </Reveal>
          </div>
...
function CTASection() {
  ...
  return (
    <section ref={ref} style={{
      ...
      <div aria-hidden style={{ position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%,-50%)', width: 640, height: 420, background: 'radial-gradient(ellipse, rgba(204,120,92,0.07) 0%, transparent 68%)', pointerEvents: 'none' }}/>
      <motion.div style={{ scale: ctaScale, opacity: ctaOp, willChange: 'transform, opacity' }}>
        ...
        <p className="cg" style={{ fontSize: 16, color: 'rgba(240,235,227,0.44)', lineHeight: 1.7, maxWidth: 440, margin: '0 auto 40px' }}>
  ...
  return (
    <div style={{ background: BG, color: INK, overflowX: 'clip', minHeight: '100vh', position: 'relative', isolation: 'isolate' }}>
      ...
      <style>{`
        ...
        .gold-btn:hover  { background: #FBBF24; box-shadow: 0 0 20px rgba(204,120,92,0.3); transform: translateY(-2px); }
        .gold-btn:active { transform: scale(0.97) translateY(0); }

        .ghost-btn {
          display: inline-flex; align-items: center; gap: 8px;
          background: transparent; color: rgba(240,235,227,0.7);
        ...
        .nav-link {
          font-family: 'DM Sans', system-ui, sans-serif; font-size: 13px; font-weight: 500;
          color: rgba(240,235,227,0.58); text-decoration: none; letter-spacing: 0.02em;
        ...
        .u-link {
          ...
          border-bottom: 1px solid rgba(204,120,92,0.45); padding-bottom: 3px;
        ...
        <div style={{ paddingTop: 64 }}>
          <section style={{ minHeight: '80vh', display: 'flex', alignItems: 'flex-end', position: 'relative', overflow: 'hidden' }}>
            ...
            <Reveal style={{ position: 'relative', zIndex: 2, padding: '0 24px 60px' }}>
              ...
              <p className="cg" style={{ fontSize: 16, fontWeight: 500, color: 'rgba(240,235,227,0.56)', maxWidth: 420, margin: 0 }}>One analyst. An entire SOC.</p>
            </Reveal>
          </section>
          <section style={{ padding: '48px 24px' }}>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(150px,1fr))', gap: 1, borderTop: '1px solid rgba(240,235,227,0.08)', borderBottom: '1px solid rgba(240,235,227,0.08)' }}>
              {STATS_DATA.map(s => (
                <div key={s.label} style={{ padding: '28px 16px 28px 0' }}>
                  ...
                  <div className="cg" style={{ fontSize: 12.5, color: 'rgba(240,235,227,0.48)', marginTop: 6 }}>{s.label}</div>
                </div>
              ))}
            </div>
          </section>
      ...
      {(isMobile || reduced) && (
        <section style={{ padding: 'clamp(60px,8vw,100px) clamp(24px,5vw,72px)' }}>
          <div style={{ maxWidth: 1240, margin: '0 auto' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(240px,1fr))', gap: 2, background: 'rgba(240,235,227,0.08)', border: '1px solid rgba(240,235,227,0.08)' }}>
              {TOOLS.map(t => (
                <div key={t.name} style={{ padding: '26px 22px', background: BG }}>
                  <div className="mono" style={{ fontSize: 10, color: 'rgba(240,235,227,0.32)', marginBottom: 12 }}>{t.layer}</div>
                  <div className="mono" style={{ fontSize: 18, fontWeight: 700, marginBottom: 8, color: INK }}>{t.name}</div>
                  <p className="cg" style={{ fontSize: 13.5, lineHeight: 1.6, color: 'rgba(240,235,227,0.5)', margin: '0 0 14px' }}>{t.desc}</p>
      ...
      <footer style={{ borderTop: '1px solid rgba(240,235,227,0.05)', padding: '28px clamp(24px,5vw,72px)' }}>
        <div style={{ maxWidth: 1240, margin: '0 auto', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 14 }}>
          ...
          <div style={{ display: 'flex', gap: 24, flexWrap: 'wrap' }}>
            <Link to="/" className="cg" style={{ color: 'rgba(240,235,227,0.26)', fontSize: 12, textDecoration: 'none' }}>Home</Link>
            <Link to="/mission" className="cg" style={{ color: 'rgba(240,235,227,0.26)', fontSize: 12, textDecoration: 'none' }}>Mission</Link>
            <a href="/app/login" target="_blank" rel="noopener noreferrer" className="cg" style={{ color: 'rgba(240,235,227,0.26)', fontSize: 12, textDecoration: 'none' }}>Platform</a>
          </div>
        </div>
      </footer>
    </div>
