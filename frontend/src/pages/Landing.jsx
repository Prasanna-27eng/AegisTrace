...
/* ─── Premium gradient orb background (Vercel / Linear style) ──────────── */
function GradientBackground() {
  return (
    <div style={{ position:'absolute', inset:0, overflow:'hidden', background:'#080C14' }}>
      ...
      {/* Dot grid overlay — extremely subtle */}
      <div style={{
...
function AIDemo() {
  ...
  const riskColor={Critical:'#EF4444',High:'#6366F1',Medium:'#EAB308',Low:'#22C55E',Clean:'#22C55E',Unknown:'#71717A'};
  ...
  return (
    <div style={{background:'rgba(240,240,248,0.02)',border:'1px solid rgba(240,240,248,0.08)',borderRadius:10,padding:24}}>
      ...
      <div style={{display:'flex',gap:10}}>
        ...
        <button onClick={()=>analyse()} disabled={loading} style={{background:'#6366F1',color:'#fff',border:'none',borderRadius:6,padding:'10px 20px',fontSize:12,fontWeight:600,cursor:'pointer',display:'flex',alignItems:'center',gap:8,...mono,whiteSpace:'nowrap'}}>
      ...
      {result&&!result.error&&(
        <div style={{marginTop:16,display:'grid',gridTemplateColumns:'1fr 1fr',gap:10}}>
          ...
          <div style={{gridColumn:'1/-1',background:'rgba(255,255,255,0.03)',border:'1px solid rgba(255,255,255,0.07)',borderRadius:8,padding:16}}>
            ...
            {result.recommendation&&<div style={{marginTop:8,fontSize:11,color:'rgba(240,240,248,0.5)',borderTop:'1px solid rgba(255,255,255,0.06)',paddingTop:8,...mono}}><strong style={{color:'#6366F1'}}>Next step:</strong> {result.recommendation}</div>}
          </div>
        </div>
      )}
...
/* ─── DATA ──────────────────────────────────────────────────────────────── */
const FEATURES = [
  { Icon:FolderSearch, title:'Case Management',     desc:'10-tab lifecycle with IOC tracking, MITRE mapping, AI analysis, AI chat, evidence vault, playbook, and timeline. Playbook state persists across sessions.',  color:'#6366F1' },
  ...
  { Icon:Monitor,      title:'Endpoint Agent v2',   desc:'Zero-dependency Python agent for Windows/Linux/Mac. Collects Sysmon events, process trees, persistence, auth logs. Heartbeat every 60s. Retry queue.',            color:'#6366F1' },
  ...
  { Icon:Bug,          title:'Malware Tools',       desc:'Base64/URL encode/decode, MD5/SHA1/SHA256/SHA512 hash generator, defang/refang, and AI-powered YARA rule generation from sample description.',                  color:'#6366F1' },
...
const HARDWARE_TOOLS = [
  { cat:'WiFi Attack', color:'#6366F1', tools:['Probe Request Analyser','Evil Twin Detector','Deauth Attack Timeline','Handshake Inspector'] },
  ...
  { cat:'Endpoint',    color:'#6366F1', tools:['Sysmon Event Parser','Process Tree Analyser'] },
...
const PROBLEMS = [
  { Icon:UserX,       title:'Identity is the new perimeter',            desc:'Attackers no longer break through firewalls — they log in with stolen credentials, hijacked tokens, or abused service accounts. Traditional SOC tools track machines. AegisTrace tracks identities.', color:'#6366F1' },
  ...
  return (
    <div style={{background:'#080C14',color:'#F0F0F8',minHeight:'100vh',cursor:'default',overflowX:'hidden'}}>
      <style>{`
        ...
        .cta-box:hover{border-color:#6366F1}
        ...
        .feat-card:hover{border-color:rgba(99,102,241,0.35);background:rgba(99,102,241,0.03);transform:translateY(-2px)}
        ...
        .hw-cat-card:hover{border-color:rgba(99,102,241,0.3);background:rgba(99,102,241,0.025)}
      ...
      <section style={{position:'relative',width:'100%',height:'100vh',overflow:'hidden'}}>
        ...
        <header className="fade-1" style={{position:'absolute',top:0,left:0,right:0,zIndex:10,display:'flex',alignItems:'center',justifyContent:'space-between',padding:'20px 48px',borderBottom:'1px solid rgba(240,240,248,0.06)'}}>
          ...
          <div style={{display:'flex',alignItems:'center',gap:12}}>
            <div style={{width:7,height:7,borderRadius:'50%',background:'#6366F1',animation:'pulse-dot 1.8s ease-in-out infinite'}}/>
            <Clock/>
            <button onClick={openApp} style={{background:'#6366F1',color:'white',border:'none',borderRadius:6,padding:'8px 18px',fontSize:11,fontWeight:600,cursor:'pointer',...mono,letterSpacing:'0.08em',textTransform:'uppercase'}}>Launch App</button>
          </div>
        </header>
        <div className="fade-2 lp-rails" style={{position:'absolute',left:20,top:'50%',transform:'translateY(-50%)',zIndex:10,display:'flex',flexDirection:'column',alignItems:'center',gap:12}}>
          ...
          <div style={{width:1,height:48,background:'rgba(99,102,241,0.3)'}}/>
        ...
        <div className="fade-2 lp-rails" style={{position:'absolute',right:20,top:'50%',transform:'translateY(-50%)',zIndex:10,display:'flex',flexDirection:'column',alignItems:'flex-end',gap:10}}>
          ...
          <div style={{width:1,height:36,background:'rgba(99,102,241,0.3)'}}/>
          <span style={{writingMode:'vertical-rl',transform:'rotate(180deg)',fontSize:10,color:'rgba(99,102,241,0.5)',letterSpacing:'0.1em',...mono}}>x{coord.x} · y{coord.y}</span>
        </div>
        <div className="fade-4 lp-hero" style={{position:'absolute',bottom:80,left:48,zIndex:10,maxWidth:900}}>
          <div style={{fontSize:10,letterSpacing:'0.45em',textTransform:'uppercase',color:'#6366F1',...mono,marginBottom:16,display:'flex',alignItems:'center',gap:8}}>
            <span style={{border:'1px solid rgba(99,102,241,0.4)',width:12,height:12,display:'inline-block'}}/>
          ...
          <h1 style={{fontFamily:'Instrument Serif,Georgia,serif',lineHeight:0.92,letterSpacing:'-0.015em',marginBottom:28}}>
            ...
            <div style={{fontSize:'clamp(3rem,8vw,9rem)',color:'#6366F1',fontStyle:'italic',fontWeight:400}}>we trace the storm.</div>
          </h1>
      ...
      <section style={{background:'#0D1117',borderTop:'1px solid rgba(240,240,248,0.06)',borderBottom:'1px solid rgba(240,240,248,0.06)'}}>
        <div className="lp-grid-4" style={{maxWidth:1200,margin:'0 auto',display:'grid',gridTemplateColumns:'repeat(4,1fr)'}}>
          {[
            ...
            {val:stats.total_iocs,     label:'IOCs Correlated',    color:'#6366F1'},
      ...
      <section id="problem" className="lp-section" style={{padding:'90px 48px',background:'#080C14',borderTop:'1px solid rgba(240,240,248,0.05)'}}>
        <div style={{maxWidth:1200,margin:'0 auto'}}>
          <div style={{marginBottom:52}}>
            <div style={{fontSize:10,color:'#6366F1',letterSpacing:'0.2em',textTransform:'uppercase',...mono,marginBottom:12}}>◇ The Problem</div>
            <h2 style={{fontFamily:'Instrument Serif,serif',fontSize:'clamp(2rem,4vw,3.4rem)',fontWeight:400,letterSpacing:'-0.02em',lineHeight:1.1,maxWidth:700}}>
              ...
              <span style={{color:'#6366F1',fontStyle:'italic'}}>than most platforms can follow.</span>
            </h2>
          ...
          {/* Stats callout */}
          <div style={{marginTop:48,padding:'28px 36px',background:'rgba(99,102,241,0.05)',border:'1px solid rgba(99,102,241,0.15)',borderRadius:10,display:'grid',gridTemplateColumns:'1fr auto 1fr auto 1fr',gap:24,alignItems:'center',flexWrap:'wrap'}}>
            <div>
              <div style={{fontFamily:'Instrument Serif,serif',fontSize:'clamp(1.8rem,3vw,2.8rem)',color:'#6366F1',fontWeight:400,fontStyle:'italic'}}>83%</div>
            ...
            <div style={{width:1,height:60,background:'rgba(99,102,241,0.2)'}}/>
            <div>
        ...
        <div style={{maxWidth:1200,margin:'0 auto'}}>
          <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:64,alignItems:'start',marginBottom:56}} className="lp-2col">
            ...
            <div style={{display:'flex',flexDirection:'column',gap:0}}>
              {[
                {from:'Case manager',          to:'Trust orchestration platform',    color:'#6366F1'},
                ...
                {from:'Manual triage',         to:'Autonomous + human-approved',     color:'#6366F1'},
      ...
      <section style={{padding:'80px 48px',background:'#080C14',borderTop:'1px solid rgba(240,240,248,0.05)'}}>
        <div style={{maxWidth:1200,margin:'0 auto'}}>
          <div style={{marginBottom:48}}>
            <div style={{fontSize:10,color:'#6366F1',letterSpacing:'0.2em',textTransform:'uppercase',...mono,marginBottom:12}}>◇ What This Platform Solves</div>
            <h2 style={{fontFamily:'Instrument Serif,serif',fontSize:'clamp(2rem,4vw,3.2rem)',fontWeight:400,letterSpacing:'-0.02em'}}>
              Built to answer the questions<br/><span style={{color:'#6366F1',fontStyle:'italic'}}>that matter most.</span>
            </h2>
          </div>
          <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(340px,1fr))',gap:20}}>
            {[
              { q:'Who or what caused this incident?', a:'AegisTrace tracks every actor — human user, service account, API key, AI agent, or device — as an identity node in a graph. Every action is linked to an identity, and every identity carries a risk score and a trust history.', Icon:Fingerprint, color:'#6366F1' },
              ...
              { q:'Are we ready for what comes next?', a:'AegisTrace is being built to remain relevant 10–15 years out — when quantum-harvested keys, prompt injection attacks, machine-to-machine abuse, and AI-native adversaries are the expected threat model, not edge cases.', Icon:TrendingUp, color:'#6366F1' },
            ].map(({q,a,Icon,color})=>(
        ...
        <div style={{maxWidth:1200,margin:'0 auto'}}>
          <div style={{display:'flex',alignItems:'flex-start',justifyContent:'space-between',gap:32,flexWrap:'wrap',marginBottom:52}}>
            <div style={{maxWidth:560}}>
              <div style={{fontSize:10,color:'#6366F1',letterSpacing:'0.2em',textTransform:'uppercase',...mono,marginBottom:10}}>◇ Session 6 — Live Now</div>
              <h2 style={{fontFamily:'Instrument Serif,serif',fontSize:'clamp(2rem,4vw,3.2rem)',fontWeight:400,letterSpacing:'-0.02em',lineHeight:1.1}}>
                Hardware attack tools.<br/><span style={{color:'#6366F1',fontStyle:'italic'}}>Analysed by AI.</span>
              </h2>
            ...
            <div style={{display:'flex',flexDirection:'column',gap:10,padding:'20px 24px',background:'rgba(99,102,241,0.05)',border:'1px solid rgba(99,102,241,0.15)',borderRadius:10}}>
              {[{Icon:Wifi,label:'WiFi/RF attacks visible alongside endpoint events'},{Icon:Cpu,label:'AI auto-parses unknown log formats — no parser needed'},{Icon:Shield,label:'Every event auto-mapped to MITRE ATT&CK technique'},{Icon:Zap,label:'One-click case creation from any hardware alert'}].map(({Icon,label})=>(
                <div key={label} style={{display:'flex',alignItems:'center',gap:10,fontSize:12,color:'rgba(240,240,248,0.7)',...mono}}>
                  <Icon size={14} style={{color:'#6366F1',flexShrink:0}}/>{label}
                </div>
              ))}
            </div>
          </div>
          ...
          <div style={{marginTop:28,textAlign:'center'}}>
            <button onClick={openApp} style={{background:'#6366F1',color:'#fff',border:'none',borderRadius:6,padding:'12px 32px',fontSize:12,fontWeight:600,cursor:'pointer',...mono,letterSpacing:'0.08em',textTransform:'uppercase'}}>Open Hardware Tools →</button>
          </div>
        </div>
      </section>
      ...
      <section id="features" className="lp-section" style={{padding:'80px 48px',background:'#080C14',borderTop:'1px solid rgba(240,240,248,0.05)'}}>
        <div style={{maxWidth:1200,margin:'0 auto'}}>
          <div style={{marginBottom:48}}>
            <div style={{fontSize:10,color:'#6366F1',letterSpacing:'0.2em',textTransform:'uppercase',...mono,marginBottom:10}}>◇ Platform Capabilities</div>
      ...
      <section id="demo" className="lp-section" style={{padding:'80px 48px',background:'#0D1117',borderTop:'1px solid rgba(240,240,248,0.05)'}}>
        <div style={{maxWidth:1100,margin:'0 auto'}}>
          <div style={{marginBottom:40,display:'flex',alignItems:'flex-end',justifyContent:'space-between',flexWrap:'wrap',gap:16}}>
            <div>
              <div style={{fontSize:10,color:'#6366F1',letterSpacing:'0.2em',textTransform:'uppercase',...mono,marginBottom:10}}>◇ Live Intelligence Demo</div>
              <h2 style={{fontFamily:'Instrument Serif,serif',fontSize:'clamp(2rem,4vw,3.2rem)',fontWeight:400,letterSpacing:'-0.02em',lineHeight:1.1}}>Try the AI<br/><span style={{color:'#6366F1',fontStyle:'italic'}}>right now.</span></h2>
            </div>
      ...
      <section id="compare" style={{padding:'80px 48px',background:'#0D1117',borderTop:'1px solid rgba(240,240,248,0.05)'}}>
        <div style={{maxWidth:1100,margin:'0 auto'}}>
          <div style={{marginBottom:44}}>
            <div style={{fontSize:10,color:'#6366F1',letterSpacing:'0.2em',textTransform:'uppercase',...mono,marginBottom:10}}>◇ Competitive Benchmark</div>
            <h2 style={{fontFamily:'Instrument Serif,serif',fontSize:'clamp(2rem,4vw,3.2rem)',fontWeight:400,letterSpacing:'-0.02em'}}>Free beats<br/><span style={{color:'#6366F1',fontStyle:'italic'}}>$50,000/year.</span></h2>
          </div>
            ...
            <table style={{width:'100%',borderCollapse:'collapse'}}>
              <thead>
                <tr>
                  ...
                  {[{name:'AegisTrace',sub:'Free',color:'#6366F1'},{name:'CrowdStrike',sub:'$~15k/yr',color:'#A78BFA'},{name:'Splunk',sub:'$~25k/yr',color:'#F97316'}].map(h=>(
      ...
      <section style={{padding:'80px 48px',background:'#080C14',borderTop:'1px solid rgba(240,240,248,0.05)'}}>
        <div style={{maxWidth:1200,margin:'0 auto'}}>
          <div style={{marginBottom:40}}>
            <div style={{fontSize:10,color:'#6366F1',letterSpacing:'0.2em',textTransform:'uppercase',...mono,marginBottom:10}}>◇ AI Engine — 100% Free via Groq</div>
            <h2 style={{fontFamily:'Instrument Serif,serif',fontSize:'clamp(2rem,4vw,3.2rem)',fontWeight:400,letterSpacing:'-0.02em'}}>Right model.<br/><span style={{color:'#6366F1',fontStyle:'italic'}}>Right task.</span></h2>
          ...
          <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(240px,1fr))',gap:12}}>
            {[
              ...
              <div key={model} style={{background:'rgba(240,240,248,0.02)',border:'1px solid rgba(240,240,248,0.07)',borderRadius:8,padding:'16px 18px'}}>
                <div style={{fontSize:11,color:'#6366F1',...mono,marginBottom:8}}>{model}</div>
      ...
      <section style={{padding:'80px 48px',background:'#0D1117',borderTop:'1px solid rgba(240,240,248,0.05)'}}>
        <div style={{maxWidth:1200,margin:'0 auto'}}>
          <div style={{marginBottom:48}}>
            <div style={{fontSize:10,color:'#6366F1',letterSpacing:'0.2em',textTransform:'uppercase',...mono,marginBottom:10}}>◇ Investigation Workflow</div>
            <h2 style={{fontFamily:'Instrument Serif,serif',fontSize:'clamp(2rem,4vw,3.2rem)',fontWeight:400,letterSpacing:'-0.02em'}}>Four steps.<br/><span style={{color:'#6366F1',fontStyle:'italic'}}>One investigation.</span></h2>
          </div>
          <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(220px,1fr))',gap:32}}>
            {[
              ...
              <div key={n}>
                <div style={{fontSize:'3.5rem',fontWeight:700,color:'rgba(99,102,241,0.12)',...mono,lineHeight:1,marginBottom:14}}>{n}</div>
      ...
      <section style={{padding:'72px 48px',background:'#080C14',borderTop:'1px solid rgba(240,240,248,0.05)'}}>
        <div style={{maxWidth:1200,margin:'0 auto'}}>
          <div style={{marginBottom:40}}>
            <div style={{fontSize:10,color:'#6366F1',letterSpacing:'0.2em',textTransform:'uppercase',...mono,marginBottom:10}}>◇ Technology Stack — All Free Tier</div>
            <h2 style={{fontFamily:'Instrument Serif,serif',fontSize:'clamp(2rem,4vw,3rem)',fontWeight:400,letterSpacing:'-0.02em'}}>Enterprise-grade.<br/><span style={{color:'#6366F1',fontStyle:'italic'}}>Zero cost.</span></h2>
          </div>
          <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(180px,1fr))',gap:12}}>
            {[
              ...
              <div key={name} style={{background:'rgba(240,240,248,0.02)',border:'1px solid rgba(240,240,248,0.07)',borderRadius:8,padding:'14px 16px'}}>
                ...
                <div style={{fontSize:11,color:'#6366F1',...mono,marginBottom:4}}>{role}</div>
      ...
      <section id="certs" style={{padding:'80px 48px',background:'#0D1117',borderTop:'1px solid rgba(240,240,248,0.05)'}}>
        <div style={{maxWidth:1200,margin:'0 auto',display:'grid',gridTemplateColumns:'1fr 1fr',gap:64,alignItems:'start'}} className="lp-2col">
          <div>
            <div style={{fontSize:10,color:'#6366F1',letterSpacing:'0.2em',textTransform:'uppercase',...mono,marginBottom:16}}>◇ About the Builder</div>
            <h2 style={{fontFamily:'Instrument Serif,serif',fontSize:'clamp(1.8rem,3.5vw,2.8rem)',fontWeight:400,letterSpacing:'-0.02em',lineHeight:1.2,marginBottom:20}}>SOC Analyst.<br/><span style={{color:'#6366F1',fontStyle:'italic'}}>Dublin, Ireland.</span></h2>
      ...
      <section style={{padding:'80px 48px 48px',borderTop:'1px solid rgba(240,240,248,0.05)',background:'#080C14'}}>
        <div style={{maxWidth:1200,margin:'0 auto'}}>
          <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',flexWrap:'wrap',gap:32,marginBottom:64}}>
            <div>
              <h2 style={{fontFamily:'Instrument Serif,serif',fontSize:'clamp(2rem,4vw,3.5rem)',fontWeight:400,letterSpacing:'-0.02em',lineHeight:1.05}}>Start your first<br/><span style={{color:'#6366F1',fontStyle:'italic'}}>investigation.</span></h2>
            ...
            <div style={{display:'flex',gap:12,flexWrap:'wrap'}}>
              <button onClick={openApp} style={{background:'#6366F1',color:'#fff',border:'none',borderRadius:6,padding:'12px 28px',fontSize:12,fontWeight:600,cursor:'pointer',...mono,letterSpacing:'0.08em',textTransform:'uppercase'}}>Launch App →</button>
